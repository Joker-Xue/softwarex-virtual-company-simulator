"""
pytest configuration files - Tests fixtures and public tools

use httpx.AsyncClient Conduct asynchronous APIIntegrated Tests.
TestsDatabaseuseDatabase，Clean data after each Tests.

compatible pytest-asyncio >=1.0 (session-scoped loop via pytest.ini)
"""
import os
...
# Set Tests environment variable（Before importing the app）
...
# Standalone Testsengine（NullPool Avoid connection pool cross-event loop issues）
...
TEST_USER_2 = {
    ...
    "full_name": "Tests user 2",
}


@pytest_asyncio.fixture(scope="session")
async def setup_db():
    """session level：Make sure the Database table exists"""
...
async def db_session(setup_db) -> AsyncGenerator[AsyncSession, None]:
    """Get an independent Database session for each Tests"""
...
async def _cleanup_user(db: AsyncSession, username: str):
    """Clean up individual users and associated data"""
    tables = [
...
async def clean_test_users(db_session: AsyncSession):
    """Clean Tests user data（Before and after Tests）"""
    for user in [TEST_USER, TEST_USER_2]:
        await _cleanup_user(db_session, user["username"])
    # Clean Testsemail verification code
    for user in [TEST_USER, TEST_USER_2]:
        try:
...
async def inject_verification_code(client: AsyncClient, email: str, code: str = "123456") -> str:
    """Inject email verification code into Database，BackVerification code value（Bypass Email Send）"""
    from app.database import get_db
    from app.models.email_verification import EmailVerificationToken

    # Write directly through engine
    async with _TestSessionLocal() as session:
...
def auth_headers(token: str) -> dict:
    """Generate authentication ask header"""
    return {"Authorization": f"Bearer {token}"}


def inject_test_captcha(suffix: str = "") -> tuple[str, str]:
    """Inject Tests verification code，Back (captcha_id, captcha_code)"""
    import time as _time
    from app.utils.captcha import _store as captcha_store
